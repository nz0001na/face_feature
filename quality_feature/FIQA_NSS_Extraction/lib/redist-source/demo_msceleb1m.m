...
src = '/media/guo/DEF23F27F23F02F7/MsCeleb1M/m.01yrkh/';
files = dir([src '*.jpg']);

fid = fopen('/media/guo/DEF23F27F23F02F7/MsCeleb1M/m.01yrkh_rqs.txt', 'a');

for i = 1:length(files)
    %% image reading and face detection
    I = imread([src files(i).name]); % change it to your own face images
    I = imresize(I, [100 60]);
...
%         toc;
%     end
        score = (polyKernelMapping([hog(crgr)*whog getGist(crgr, [], param)*wgst gabor(crgr, fb_real, fb_imag)*wgab ...
            lbp(crgr)*wlbp cnn(crgr)*wnn])*wk - 3.75)*100/3;
        score = round(score);
        fprintf(fid, '%s,%d\n', files(i).name, score);
        fprintf('%s,%d\n', files(i).name, score);
