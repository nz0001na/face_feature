...
for i=1:size(dirlist,1)
    ...
    for j=1:len
        ...
        img = imread([src_path folder '/' img_name]);
        if(size(img,3)>1)
            img = rgb2gray(img);
        end        
        feature = bsif(img ,ICAtextureFilters, 'h');
